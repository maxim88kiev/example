...
public class PingPongGameEngine implements Runnable, MouseMotionListener, KeyListener, GameConstants {
	...
	private int ballX; //координата Х мяча
	private int ballY; //координата У мяча
	...
	//конструктор. содержит ссылку на обьект стола
	PingPongGameEngine (PingPongGreenTable greenTable){
	...
	//Обязательные методы из инт ерфейса MouseMotionListener
	...
	//бязательный метод run() из интерфейса Runnable
	...
	public void run(){
		...
		while (true){
			if(ballServed){ //если мяч движется
			  ...
			  //шаг 5. обновить счет, если мяч в зеленой области, но не движетья
			  if (isBallOnTheTable()){
			  if (ballX > BALL_MAX_X){
				  computerScore++;
				  displayScore();
